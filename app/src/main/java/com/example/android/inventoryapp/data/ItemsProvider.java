...
public class ItemsProvider extends ContentProvider {
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (match) {
            case INVENTORY:
                ...
                cursor = database.query(InventoryEntry.TABLE_NAME, projection, selection, selectionArgs, null, null,
                        sortOrder);
    ...
    private Uri insertItem(Uri uri, ContentValues values) {
        ...
        if (name == null) {
            throw new IllegalArgumentException("A fruit is required");
        }

        // If the weight is provided, check that it's greater than or equal to 0 kg
        Integer quantity = values.getAsInteger(InventoryEntry.COLUMN_QUANTITY);
        if (quantity != null && quantity < 0) {
            throw new IllegalArgumentException("Qauntity is required");
        }
    ...
    private int updatePet(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(InventoryEntry.COLUMN_PRICE)) {
            // Check that the weight is greater than or equal to 0 kg
            Integer weight = values.getAsInteger(InventoryEntry.COLUMN_PRICE);
            if (weight != null && weight < 0) {
                throw new IllegalArgumentException("Pet requires valid weight");
            }
        }
